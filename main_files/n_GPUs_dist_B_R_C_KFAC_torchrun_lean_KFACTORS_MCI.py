...
def main(world_size, args):
    ...
    optimizer =  B_R_C_KFACOptimizer(model, rank = rank, world_size = world_size, batch_size = args.batch_size,
    ...
    stored_metrics_object = train_n_epochs(model, optimizer, loss_fn, train_loader, train_sampler, val_loader, 
                                           schedule_function, 
                                           args, len_train_loader, rank, world_size, optim_type = 'BRC-KFAC')
    ...
    if rank == 0 and args.store_and_save_metrics:
        ...
        stored_metrics_object.save_metrics( metrics_save_path = args.metrics_save_path, dataset = args.dataset, 
                                           net_type = args.net_type, solver_name = 'BRC', nGPUs = args.world_size,
    ...
    print('######### Finished Running BRC-KFAC at seed = {} ######################################################'.format(args.seed))
...
if __name__ == '__main__':
    ...
    print('\n ######### Started Running BRC-KFAC at seed = {} ######################################################'.format(args.seed))
