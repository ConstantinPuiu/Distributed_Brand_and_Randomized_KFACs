...
def main(world_size, args):
    ...
    args, TInv_schedule, TCov_schedule, brand_update_multiplier_to_TCov_schedule, \
        KFAC_damping_schedule, KFAC_damping = adjust_args_for_schedules(args, solver_name = 'B-KFAC')
    ################################ END SCHEDULES ###################################################################
    ...
    optimizer =  B_KFACOptimizer(model, rank = rank, world_size = world_size, batch_size = args.batch_size,
                                ...
                                # specific to B-kfac: brand specific parameters
    ...
    stored_metrics_object = train_n_epochs(model, optimizer, loss_fn, train_loader, train_sampler, val_loader, 
                                           schedule_function, 
                                           args, len_train_loader, rank, world_size, optim_type = 'B-KFAC')
