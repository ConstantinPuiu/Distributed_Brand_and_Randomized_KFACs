...
from Distributed_Brand_and_Randomized_KFACs.solvers.track_time_distributed_R_kfac_lean_Kfactors_batchsize import R_KFACOptimizer
...
def main(world_size, args):
    ...
    #### save times to pickle
    if optimizer.work_alloc_propto_RSVD_cost == False:
        wa = 0
    else:
        wa = 1
    ...
    if optimizer.rank == 0:
        ...
        with open('/data/math-opt-ml/chri5570/time_pickles/{}/kfactor_size_G.pickle'.format(folder_exact), 'wb') as handle:
            pickle.dump(ad_hoc_dict_for_odule_size_eff_storage_G, handle, protocol=pickle.HIGHEST_PROTOCOL)
