...
from Distributed_Brand_and_Randomized_KFACs.solvers.solver_utils.B_R_C_utils_for_C import perform_C_correction

class B_R_C_KFACOptimizer(optim.Optimizer):
    def __init__(self,
                 model,
                 rank, world_size,
                ...
                correction_multiplier_TCov = 5,
                brand_corection_dim_frac = 0.2
                ):
        ...
        super(B_R_C_KFACOptimizer, self).__init__(model.parameters(), defaults)
        ...
        ##### BR specific ###################
        self.B_R_period = B_R_period
        self.LL_modules_to_have_R_done_for_this_rank_A = {}
    ...
    def _save_input(self, module, input):
        ...
        if torch.is_grad_enabled() and self.steps % self.TCov == 0:
            ...
            if self.steps == 0:
                ...
                ##### initialize m_aa or associated tensors ######################
            ...
            #########################################################
            ### update m_aa for the modules responsible for doing the R-update of the B-modules
    ...
    def _save_grad_output(self, module, grad_input, grad_output):
        ...
        if self.acc_stats and self.steps % self.TCov == 0:
            if self.steps == 0:
                ...
                ##### initialize m_gg or associated tensors ######################
    ...
    def _update_inv(self, m):
        ...
        def do_rsvd_decomp_for_A(adaptable_rank, rank_adaptation_multiplier, T_ref, current_ranks, basic_rank):
            ...
            # adaptable_rank can be either self.adaptable_rsvd_rank or self.adaptable_B_rank
            # rank_adaptation_multiplier can be iether B_rank_adaptation_T_brand_updt_multiplier for LL or rsvd_rank_adaptation_TInv_multiplier for CaSL
            # T_ref can be T_Inv for CaSL or T_brand_updt for LL
            # the 2 make sure the 1st initialization of current_rank was performed
            # current_ranks can be either current_B_ranks_a or current_rsvd_ranks_a
        ...
        if (m in list_to_check_in_A) and (m in self.size_0_of_CaSL_Kfactors_A): # if module is allocated to this GPU and is CaSL
            ...
            do_rsvd_decomp_for_A(adaptable_rank = self.adaptable_rsvd_rank, rank_adaptation_multiplier = self.rsvd_rank_adaptation_TInv_multiplier, 
                                 T_ref = self.TInv, current_ranks = self.current_rsvd_ranks_a, basic_rank = self.rsvd_rank)
        ...
        if (m in self.size_0_of_LL_Kfactors_A) and ( self.steps % (self.TInv * self.B_R_period) == 0 ) and self.steps > self.TInv:
            if m in self.LL_modules_to_have_R_done_for_this_rank_A[self.rank]:
                ...
                do_rsvd_decomp_for_A(adaptable_rank = self.adaptable_B_rank, rank_adaptation_multiplier = self.B_rank_adaptation_T_brand_updt_multiplier,
                ...
                if self.Q_a[m].shape[0] == actual_rank:
                    ...
                    self.Q_a[m] = 0 * self.aa_for_reinit[m][:,:actual_rank]
                
                if self.d_a[m].shape[0] == actual_rank:
                    self.d_a[m] = 0 * self.d_a[m]
                else:
        ...
        def do_rsvd_decomp_for_G(adaptable_rank, rank_adaptation_multiplier, T_ref, current_ranks, basic_rank):
            ...
            # T_ref can be T_Inv for CaSL or T_brand_updt for LL
            # the 2 make sure the 1st initialization of current_rank was performed
            # current_ranks can be either current_B_ranks_g or current_rsvd_ranks_g
            # basic_rank can be either self.rsvd_rank or self.brand_r_target : it's the rank when no rank-adaptation is done
            ...
            if self.dist_comm_for_layers_debugger:
                print('RANK {} WORLDSIZE {}. computed EVD of module {} \n'.format(self.rank, self.world_size, m))
                print('The shapes are Q_g.shape = {}, d_g.shape = {}'. format(self.Q_g[m].shape, self.d_g[m].shape))
        #########    
        
        if (m in list_to_check_in_G) and (m in self.size_0_of_CaSL_Kfactors_G): # if module is allocated to this GPU and is CaSL
            do_rsvd_decomp_for_G(adaptable_rank = self.adaptable_rsvd_rank, rank_adaptation_multiplier = self.rsvd_rank_adaptation_TInv_multiplier, 
                                 T_ref = self.TInv, current_ranks = self.current_rsvd_ranks_g, basic_rank = self.rsvd_rank)
        ...
        if (m in self.size_0_of_LL_Kfactors_G) and ( self.steps % (self.TInv * self.B_R_period) == 0 ) and self.steps > self.TInv:
            if m in self.LL_modules_to_have_R_done_for_this_rank_G[self.rank]:
                ...
                if self.Q_g[m].shape[0] == actual_rank:
                    ...
                    self.Q_g[m] = 0 * self.gg_for_reinit[m][:,:actual_rank]
                
                if self.d_g[m].shape[0] == actual_rank:
                    self.d_g[m] = 0 * self.d_g[m]
                else:
    ...
    def step(self, epoch_number, error_savepath, closure = None):
        ...
        if self.steps == 0 and self.work_alloc_propto_RSVD_and_B_cost == True:
            ...
            self.CaSL_modules_for_this_rank_A, self.CaSL_modules_for_this_rank_G, sum_R_loads_each_worker = allocate_RSVD_inversion_work_same_fixed_r(number_of_workers = self.world_size, 
            ...
            ############## Reallocate B-update work (LL layers) ############################
            ...
            ############ END REALLOCATE B-update WORK (LL LAYERS) #########################
            
            ############## Reallocate R-update of LL layers for the R-step of B-modules (the B-R specific step) #####################
            self.LL_modules_to_have_R_done_for_this_rank_A, self.LL_modules_to_have_R_done_for_this_rank_G, _ = allocate_RSVD_inversion_work_same_fixed_r(number_of_workers = self.world_size, 
                                                                                    size_0_of_all_Kfactors_G = self.size_0_of_LL_Kfactors_G,
                                                                                    size_0_of_all_Kfactors_A = self.size_0_of_LL_Kfactors_A,
                                                                                    target_rank_RSVD = self.rsvd_rank, 
                                                                                    already_alloc_time_list = sum_R_loads_each_worker)
            ######### END : Reallocate R-update of LL layers for the R-step of B-modules (the B-R specific step) ####################
