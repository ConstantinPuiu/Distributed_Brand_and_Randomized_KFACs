...
def srevd_lowrank(M, oversampled_rank, target_rank, niter, GPU_rank, start_matrix = None):
    #generate random matrix
    devicce =  torch.device('cuda:{}'.format(GPU_rank))
    n = M.shape[0]
...
### end helper function, now define class

class KFACOptimizer_measuring_rNLA_error(optim.Optimizer):
    def __init__(self,
                 model,
        ...
        super(KFACOptimizer_measuring_rNLA_error, self).__init__(model.parameters(), defaults)
        ...
        self.savepath_rnla_error = savepath_rnla_error
        self.rsvd_total_err_A = {}; self.rsvd_total_err_G = {} 
        self.srevd_total_err_A = {}; self.srevd_total_err_G = {}
        self.actual_attained_tr_error_A = {}; self.actual_attained_tr_error_G = {}
        self.target_rank_A = {}; self.target_rank_G = {}
        self.oversampled_rank_A = {}; self.oversampled_rank_G = {}
        self.layer_number = {}; self.layer_type = {}
        
        self.rel_tr_err = rel_tr_err
        # save filename: self.savepath_rnla_error + '/espectrum_raw_kfactor_A_netScaleFactor_{}_layer_{}_iter_{}'.format(Network_scalefactor, layer_number, layer_type, self.steps)
        # save filename: self.savepath_rnla_error + '/espectrum_raw_kfactor_G_netScaleFactor_{}_layer_{}_iter_{}'.format(Network_scalefactor, layer_number, layer_type, self.steps)
        # save filename: self.savepath_rnla_error + '/espectrum_EA_kfactor_A_netScaleFactor_{}_layer_{}_iter_{}'.format(Network_scalefactor, layer_number, layer_type, self.steps)
        # save filename: self.savepath_rnla_error + '/espectrum_EA_kfactor_G_netScaleFactor_{}_layer_{}_iter_{}'.format(Network_scalefactor, layer_number, layer_type, self.steps)
    ...
    def get_r_for_rel_tr_err(self, d, rel_tr_error):
        # ensure d is descending
        # if last returned element is 1, then we shouldn't do rsvd as we'd normally take all the modes
        d = - torch.sort(-d)[0] 
        errors = d / d[0]
    ...
    def _update_inv(self, m):
        ...
        if m in self.modules_for_this_rank_A[self.rank]:
            ...
            self.d_a[m], self.Q_a[m] = torch.linalg.eigh(self.m_aa[m] + \
                                                         torch.diag(self.m_aa[m].new(self.m_aa[m].size(0)).fill_(1 * 1e-08)),
                                                         UPLO = 'U') #deprecated # torch.symeig( self.m_aa[m], eigenvectors=True)
            self.d_a[m] = self.d_a[m] - 1e-08
            self.d_a[m].mul_((self.d_a[m] > 0).float())
            
            ################ for rNLA error #######################
            r, actual_attained_tr_error, flagg = self.get_r_for_rel_tr_err(self.d_a[m],  self.rel_tr_err)
            r_oversampling_param = min(10, self.m_aa[m].shape[0] - r)
            
            if flagg == 0:
                # rsvd
                _, d_a_rsvd, Q_a_rsvd = torch.svd_lowrank(self.m_aa[m], q = r + r_oversampling_param, niter = 4, M = None) # this is rsvd
                Q_a_rsvd = Q_a_rsvd[:,:r]
                d_a_rsvd = d_a_rsvd[:r]
                
                rsvd_total_err = torch.linalg.norm(self.m_aa[m] - ((Q_a_rsvd * d_a_rsvd) @ Q_a_rsvd.T) , ord = 2) / \
                                    torch.linalg.norm(self.m_aa[m], ord = 2)
                del d_a_rsvd, Q_a_rsvd
                
                # srevd
                d_a_srevd, Q_a_srevd = srevd_lowrank(M = self.m_aa[m], oversampled_rank = r + r_oversampling_param,
                                                     target_rank = r, niter = 4, GPU_rank = self.rank,
                                                     start_matrix = None)
                srevd_total_err = torch.linalg.norm(self.m_aa[m] - ((Q_a_srevd * d_a_srevd) @ Q_a_srevd.T) , ord = 2) / \
                                    torch.linalg.norm(self.m_aa[m], ord = 2)
                
                # append
                if m in self.rsvd_total_err_A.keys():
                    self.rsvd_total_err_A[m].append( rsvd_total_err )
                else:
                    self.rsvd_total_err_A[m] = [ rsvd_total_err ]
                
                if m in self.srevd_total_err_A.keys():
                    self.srevd_total_err_A[m].append( srevd_total_err )
                else:
                    self.srevd_total_err_A[m] = [ srevd_total_err ]
                
                if m in self.actual_attained_tr_error_A.keys():
                    self.actual_attained_tr_error_A[m].append(actual_attained_tr_error)
                    self.target_rank_A[m].append(r)
                    self.oversampled_rank_A[m].append(r_oversampling_param)
                else:
                    self.actual_attained_tr_error_A[m] = [actual_attained_tr_error]
                    self.target_rank_A[m] = [r]
                    self.oversampled_rank_A[m] = [r_oversampling_param]
                            
                ##### save rNLA and total error of EA Kfactor Gamma #################
                filename = self.savepath_rnla_error  + '/RSVD_error_A_layer_{}_LayerType_{}.pt'.format(
                            self.layer_number[m], self.layer_type[m])
                torch.save(self.rsvd_total_err_A[m], filename)
                
                filename = self.savepath_rnla_error + '/SREVD_error_A_layer_{}_LayerType_{}.pt'.format(
                            self.layer_number[m], self.layer_type[m])
                torch.save(self.srevd_total_err_A[m], filename)
                
                filename = self.savepath_rnla_error + '/truncation_error_A_layer_{}_LayerType_{}.pt'.format(
                            self.layer_number[m], self.layer_type[m])
                torch.save(self.actual_attained_tr_error_A[m], filename)
                
                filename = self.savepath_rnla_error + '/r_A_layer_{}_LayerType_{}.pt'.format(
                            self.layer_number[m], self.layer_type[m])
                torch.save(self.target_rank_A[m], filename)
                
                filename = self.savepath_rnla_error + '/r_o_A_layer_{}_LayerType_{}.pt'.format(
                            self.layer_number[m], self.layer_type[m])
                torch.save(self.oversampled_rank_A[m], filename)
                #### save rNLA and total error of EA Kfactor Gamma ##################
            ################ END: for rNLA error ##################
        ...
        if m in self.modules_for_this_rank_G[self.rank]:  
            ...
            ################ for rNLA error #######################
            r, actual_attained_tr_error, flagg = self.get_r_for_rel_tr_err(self.d_g[m],  self.rel_tr_err)
            r_oversampling_param = min(10, self.m_gg[m].shape[0] - r)
            
            if flagg == 0:
                # rsvd
                _, d_g_rsvd, Q_g_rsvd = torch.svd_lowrank(self.m_gg[m], q = r + r_oversampling_param, niter = 4, M = None) # this is rsvd
                Q_g_rsvd = Q_g_rsvd[:,:r]
                d_g_rsvd = d_g_rsvd[:r]
                
                rsvd_total_err = torch.linalg.norm(self.m_gg[m] - ((Q_g_rsvd * d_g_rsvd) @ Q_g_rsvd.T) , ord = 2) / \
                                    torch.linalg.norm(self.m_gg[m], ord = 2)
                del d_g_rsvd, Q_g_rsvd
                
                # srevd
                d_g_srevd, Q_g_srevd = srevd_lowrank(M = self.m_gg[m], oversampled_rank = r + r_oversampling_param,
                                                     target_rank = r, niter = 4, GPU_rank = self.rank, 
                                                     start_matrix = None)
                srevd_total_err = torch.linalg.norm(self.m_gg[m] - ((Q_g_srevd * d_g_srevd) @ Q_g_srevd.T) , ord = 2) / \
                                    torch.linalg.norm(self.m_gg[m], ord = 2)
                
                # append
                if m in self.rsvd_total_err_G.keys():
                    self.rsvd_total_err_G[m].append( rsvd_total_err )
                else:
                    self.rsvd_total_err_G[m] = [ rsvd_total_err ]
                
                if m in self.srevd_total_err_G.keys():
                    self.srevd_total_err_G[m].append( srevd_total_err )
                else:
                    self.srevd_total_err_G[m] = [ srevd_total_err ]
                
                if m in self.actual_attained_tr_error_G.keys():
                    self.actual_attained_tr_error_G[m].append(actual_attained_tr_error)
                    self.target_rank_G[m].append(r)
                    self.oversampled_rank_G[m].append(r_oversampling_param)
                else:
                    self.actual_attained_tr_error_G[m] = [actual_attained_tr_error]
                    self.target_rank_G[m] = [r]
                    self.oversampled_rank_G[m] = [r_oversampling_param]
                
           
                
                ##### save rNLA and total error of EA Kfactor Gamma #################
                filename = self.savepath_rnla_error + '/RSVD_error_G_layer_{}_LayerType_{}.pt'.format(
                            self.layer_number[m], self.layer_type[m])
                torch.save(self.rsvd_total_err_G[m], filename)
                
                filename = self.savepath_rnla_error + '/SREVD_error_G_layer_{}_LayerType_{}.pt'.format(
                            self.layer_number[m], self.layer_type[m])
                torch.save(self.srevd_total_err_G[m], filename)
                
                filename = self.savepath_rnla_error  + '/truncation_error_G_layer_{}_LayerType_{}.pt'.format(
                            self.layer_number[m], self.layer_type[m])
                torch.save(self.actual_attained_tr_error_G[m], filename)
                
                filename = self.savepath_rnla_error + '/r_G_layer_{}_LayerType_{}.pt'.format(
                            self.layer_number[m], self.layer_type[m])
                torch.save(self.target_rank_G[m], filename)
                
                filename = self.savepath_rnla_error + '/r_o_G_layer_{}_LayerType_{}.pt'.format(
                            self.layer_number[m], self.layer_type[m])
                torch.save(self.oversampled_rank_G[m], filename)
                #### save rNLA and total error of EA Kfactor Gamma ##################
            ################ END: for rNLA error ##################
    ...
    def step(self, epoch_number, error_savepath, closure = None):
        ...
        if self.steps % self.TInv == 0:
            for m in self.modules:
                self._update_inv(m) # for the EA k-factors we save directly in the old function ( no need for a new function)
