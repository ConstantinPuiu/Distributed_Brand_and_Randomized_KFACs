...
def fct_split_list_of_modules(modules, world_size):
    modules_for_this_rank = {}
    how_many_per_GPU = len(modules) / world_size
    int_part = round(how_many_per_GPU)
    if int_part > how_many_per_GPU: int_part = int_part - 1 # if round() returned ceil(), set it to floor()
    remainder = how_many_per_GPU - int_part
    print(int_part, remainder)
    accumulated_leftover = 0; current_plus_one_threshold = 1
    hmtso = 0; final_index = 0 # how_many_times_surpassed_one
    for Gri in range(0, world_size-1): # all but the last one
        accumulated_leftover = accumulated_leftover + remainder
        if accumulated_leftover < current_plus_one_threshold:
            modules_for_this_rank[Gri] = modules[Gri*int_part + hmtso: (Gri+1)* int_part + hmtso]
            final_index = (Gri+1)* int_part + hmtso
        else:
            current_plus_one_threshold += 1
            modules_for_this_rank[Gri] = modules[Gri*int_part + hmtso : (Gri+1)* int_part + hmtso + 1]
            hmtso += 1
            final_index = (Gri+1)* int_part + hmtso
    modules_for_this_rank[world_size - 1] = modules[final_index:]
    return modules_for_this_rank
