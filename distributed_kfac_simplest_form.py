...
class KFACOptimizer(optim.Optimizer):
    ...
    def _save_input(self, module, input):
        if torch.is_grad_enabled() and self.steps % self.TCov == 0:
            ...
            if self.K_fac_incoming_info_debugger_mode:
    ...
    def _update_inv(self, m):
        ...
        if m in self.modules_for_this_rank[self.rank]:
            ...
            self.d_a[m] = 0 * self.m_aa[m][0,:]
            self.Q_a[m] = 0 * self.m_aa[m]
            self.d_g[m] = 0 * self.m_gg[m][0,:]
            self.Q_g[m] = 0 * self.m_gg[m]
            #else: # else it'ssimpler to set what we had before to zero by multyplying the existing by 0
            #    self.d_a[m] = 0 * self.d_a[m]; self.d_g[m] = 0 * self.d_g[m]
            #    self.Q_a[m] = 0 * self.Q_a[m]; self.Q_g[m] = 0 * self.Q_g[m]
    ...
    def step(self, epoch_number, error_savepath, closure = None):
        ...
        ### If the AA^T and GG^T statistics have been updated locally, allreduce them again! ########
        if self.steps % self.TCov == 0:
            # THIS IS A TERIBLE COMMUNICATION BECAUSE IT MUST INITIATE HANDSHAKE FOR ALL MODULES AGAIN TWICE!!!
            # FIX: MAKE THIS MORE EFFICIENT... LOOK AT THE GUY's code
            for m in self.modules:
                if self.dist_communication_2nd_version_debugger:
                    print('RANK {} WORLDSIZE {}. At module {}. LOCAL moving avg of GG^T ={}'.format(self.rank, self.world_size, m, self.m_gg[m]))
                self.m_aa[m] = self.m_aa[m] / self.world_size
                self.m_gg[m] = self.m_gg[m] / self.world_size
                dist.all_reduce(self.m_aa[m], dist.ReduceOp.SUM, async_op = False)
                dist.all_reduce(self.m_gg[m], dist.ReduceOp.SUM, async_op = False)
                if self.dist_communication_2nd_version_debugger:
                    print('RANK {} WORLDSIZE {}. At module {}. ALLREDUCED moving avg of GG^T ={}'.format(self.rank, self.world_size, m, self.m_gg[m]))
                    print('RANK {} WORLDSIZE {}. At module {}. DONE doing ALLREDUCE(AA^T,GG^T)\n at step {}\n'.format(self.rank, self.world_size, m, self.steps))
        #############################################################################################
