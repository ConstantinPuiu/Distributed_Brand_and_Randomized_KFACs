...
class B_R_KFACOptimizer(optim.Optimizer):
    def __init__(self,
        ...
        self.dist_comm_for_layers_debugger = True #False
    ...
    def step(self, epoch_number, error_savepath, closure = None):
        ...
        for m in self.modules:
            ...
            if (self.steps % self.TInv == 0 and (not isinstance(m, nn.Linear)) ) or (self.steps % (self.TCov * self.brand_update_multiplier_to_TCov) == 0 and isinstance(m, nn.Linear)):
