...
def get_network(network, dropout = False, **kwargs):
    ...
    if dropout == True :
        if network == 'vgg16_bn_less_maxpool':
            ...
            Net.classifier = nn.Sequential(
                                                  nn.Linear(int(512 * 1 * kwargs.get('Network_scalefactor', 1)), int(256 * kwargs.get('Network_scalefactor', 1) ) ),
                                                  nn.ReLU(inplace = False),
                                                  nn.Dropout(), # p =0.5  by default
                                                  nn.Linear(int(256 * kwargs.get('Network_scalefactor', 1)), kwargs.get('num_classes', 10) ),
                                              )
        else:
